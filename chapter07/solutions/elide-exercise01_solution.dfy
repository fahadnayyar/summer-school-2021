//#title Synchronous KV Store With Application Correspondence
//#desc 
//#desc a specification (a logically-centralized abstract map).
//#desc TODO diff off of chapter06/solutions/exercise01_solution.dfy
...
module MapSpec {
  ...
  // These action labels support Application Correspondence: if the protocol wants
  // to deploy a QueryOp, it's going to have to cough up a QueryLabel for
  // the transition. This lets us enforce that application-visible operations match
  // the spec actions without having the "code inspector" have to read the refinement
  // proof and abstraction function.
  datatype ActionLabel =
  ...
  predicate InsertOp(v:Variables, v':Variables, lbl:ActionLabel)
    requires lbl.InsertLabel?
  {
    && lbl.InsertLabel?
    ...
    && v'.mapp == v.mapp[lbl.key := lbl.value]
  }

  predicate QueryOp(v:Variables, v':Variables, lbl:ActionLabel)
    requires lbl.QueryLabel?
  {
    && lbl.key in AllKeys()
    ...
    && lbl.key in v.mapp
    && lbl.output == v.mapp[lbl.key]
    && v' == v  // no change to map state
  }

  // In this spec, the ActionLabel has everything we need to function as a JayNF
  // receipt (there's no nondeterminism that we didn't also want to expose as
  // application-user-visible). So we don't need an exists or JayNF datatype.

  // To enforce application correspondence, we label each Next step with the
  // information that's visible to the application user. This ensures that,
  // if the protocol purports to have done, say, a query step (by providing
  // a QueryLabel as this argument), it can't try to refine its Next to
  // an InsertOp or a NoOp.
  predicate Next(v: Variables, v': Variables, actionLabel: ActionLabel)
  {
    match actionLabel
